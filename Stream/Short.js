...
function app_get_host_name(e) {
  return null == e ||
    "" === e ||
    e.match(/^\#/) ||
    -1 === (e = app_get_url(e)).href.search(/^http[s]?:\/\//)
    ? ""
    : e.href.split("/")[2].split(":")[0].toLowerCase();
}
...
function app_match_wildcard_domain(e, n) {
  ...
  for (i = 0; i < a.length; i++)
    if (n.substr(-1 * a[i].length) === a[i]) return true;
  return false;
}
  ...
  if (linkUrl) {
    ...
    if (app_domains && app_domain_exist(app_domains, hostname)) {
      var apiUrl =
        ...
        "&url=" +
        app_base64_encode(linkUrl) +
        "&type=" +
        encodeURIComponent(linkType);
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  var container = document.getElementById(
    "disable-link-modification-container"
  );
  if (!container) {
